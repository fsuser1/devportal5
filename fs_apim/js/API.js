...
(function ($) {
    ...
    function API(apis, expandedapis) {
        ...
        function populateCodeSnippets(api, path, verb) {
            var id = "apis_" + cleanUpClassName(api.info["x-fs-name"] + api.info["version"]) + "_paths_" + cleanUpKey(path) + "_" + verb;
            var operation = api.paths[path][verb];
            var config;
            var tester = self.testers[api.info["x-fs-name"] + api.info["version"] + "_" + verb + "_" + path];
            if (tester) {
        ...
        function generateExamples(api, path, verb) {
            var exampleid = "exampleresponse_apis_" + cleanUpClassName(api.info["x-fs-name"] + api.info["version"]) + "_paths_" + cleanUpKey(path) + "_" + verb;

            function hideTab(api, path, verb) {
                var cleanedapiname = cleanUpClassName(api.info["x-fs-name"] + api.info["version"]);
        ...
        function populateCodeSnippetsTimeout(expanded, path, verb) {
            ...
            var id = "content-apis_" + cleanUpClassName(expanded.info["x-fs-name"] + expanded.info["version"]) + "_paths_" + cleanUpKey(path) + "_" + verb;
            var requestForm = $("." + id + " form[name='request_" + cleanUpClassName(expanded.info["x-fs-name"] + expanded.info["version"]) + "_" + cleanUpKey(path) + "_" + verb + "']");
        ...
        self.apis.forEach(function (api) {
            ...
            self.expandedapis.forEach(function (expanded) {
                if (expanded.info['x-fs-name'] && expanded.info['x-fs-name'] == api.info['x-fs-name'] && expanded.info['version'] && expanded.info['version'] == api.info['version']) {
            ...
            self.expandedapis.forEach(function (api) {
                Object.keys(api.paths).forEach(function (path) {
                    ...
                    Object.keys(pathObject).forEach(function (verb) {
                        if (["PUT", "POST", "GET", "DELETE", "OPTIONS", "HEAD", "PATCH"].indexOf(verb.toUpperCase()) != -1) {
                            self.testers[api['info']['x-fs-name'] + api['info']['version'] + "_" + verb + "_" + path] = new Test({
                                    api: api,
                                    path: path,
                                    verb: verb,
        ...
        function watchEndpoint() {
            $(".endpointSelect").change(function (event) {
                ...
                if (endpoint) {
                    ...
                    self.expandedapis.forEach(function (expanded) {
                        var cleanedapiname = cleanUpClassName(expanded.info["x-fs-name"] + expanded.info["version"]);
                            ...
                            Object.keys(expanded.paths).forEach(function (path) {
                                Object.keys(expanded.paths[path]).forEach(function (verb) {
                                    if (["PUT", "POST", "GET", "DELETE", "OPTIONS", "HEAD", "PATCH"].indexOf(verb.toUpperCase()) != -1) {
                                        ...
                                        populateCodeSnippetsTimeout(expanded, path, verb);
                                        var exampleid = cleanUpClassName(expanded.info["x-fs-name"] + expanded.info["version"]) + "_paths_" + cleanUpKey(path) + "_" + verb;
                                        var tryid = cleanUpClassName(expanded.info["x-fs-name"] + expanded.info["version"]) + "_" + cleanUpKey(path) + "_" + verb;
    ...
    API.prototype.getEndpoint = function (api) {
        ...
        } else if (api && api['x-fs-endpoints'] && api['x-fs-endpoints'][0] && api['x-fs-endpoints'][0].endpointUrl) {
            return api['x-fs-endpoints'][0].endpointUrl;
    ...
    function Test(operation, api, apiObj) {
        ...
        var id = "content-apis_" + cleanUpClassName(api.info["x-fs-name"] + api.info["version"]) + "_paths_" + cleanUpKey(self.operation.path) + "_" + self.operation.verb;
        self.responseSection = $("." + id + " #response_" + cleanUpClassName(api.info["x-fs-name"] + api.info["version"]) + '_' + cleanUpKey(self.operation.path) + "_" + self.operation.verb);
        self.requestForm = $("." + id + " form[name='request_" + cleanUpClassName(api.info["x-fs-name"] + api.info["version"]) + "_" + cleanUpKey(self.operation.path) + "_" + self.operation.verb + "']");
    ...
    Test.prototype.test = function () {
        ...
        if (this.config.requiresClientId && this.config.clientIdLocation == "header" && requestForm.apimClientId) {
            if (this.config.clientIdName) {
                ...
                headers["X-FS-Client-Id"] = requestForm.apimClientId;
            }
        }
        if (this.config.requiresClientSecret && this.config.clientSecretLocation == "header" && requestForm.apimClientSecret) {
            if (this.config.clientSecretName) {
                ...
                headers["X-FS-Client-Secret"] = requestForm.apimClientSecret;
            }
        }
        ...
        if (this.operation.definition['x-fs-soap'] && this.operation.definition && this.operation.definition.operationId && targetUrl.endsWith("/" + this.operation.definition.operationId)) {
        ...
        function stringifyRequest() {
            ...
            if (self.xhrOpts && self.xhrOpts.headers) {
                Object.keys(self.xhrOpts.headers).forEach(function (headerName) {
                    if (headerName == 'X-FS-Client-Secret' || (self.config.clientSecretName && headerName == self.config.clientSecretName)) {
