...
drupal_add_js(drupal_get_path('module', 'fs_apim') . '/js/Product.js', array(
  'weight' => 4
));
...
drupal_add_js(drupal_get_path('module', 'fs_apim') . '/js/subscribe.js', array(
  'weight' => 8
));
        ...
        <ul>
          ...
          <li
            ...
            <?php if (isset($apinode->status) && $apinode->status == 1) : ?>
              <a
                href="<?php print url('productapi/' . $node->nid . '/' . fs_apim_base64_url_encode($apinode->api_ref[$apinode->language][0]['value'])) ?>"
          ...
          <section class="documentation navigate-product">
            <div class="documentationContent fsSelfClear">
              <div class="titleHeader">
                        ...
                        <div class="field-items">
                          <figure class="clearfix field-item even">
                            <img typeof="foaf:Image" class="image-style-none"
                                 src="<?php print file_create_url(drupal_get_path('module', 'fs_apim') . '/images/icons/product/' . product_random_image($node->title)); ?>"
                ...
                <h1 class="name">
                  <?php print fs_apim_get_translated_string($product, ['info'], 'title'); ?>
                  <?php if ($showversions == 1): ?>
                    <span
              ...
              <div class="info">
                ...
                <?php if (isset($product['info']['description']) && !empty($product['info']['description'])) : ?>
                  <div>
                    ...
                    <div class="markdown"
                         id='product_description'><?php print fs_apim_markdown_field(fs_apim_get_translated_string($product, ['info'], 'description')); ?></div>
                  </div>
                <?php endif; ?>
                ...
                <?php if (isset($product['info']['license']) && isset($product['info']['license']['url']) && isset($product['info']['license']['name']) && !empty($product['info']['license']['url']) && !empty($product['info']['license']['name'])) : ?>
                  <div>
                    ...
                    <div><a
                        ...
                        target='_blank'><?php print fs_apim_get_translated_string($product['info']['license'], array(), 'name'); ?></a></div>
                  </div>
                <?php endif; ?>
                <?php if (isset($product['info']['termsOfService']) && !empty($product['info']['termsOfService'])) : ?>
                  <div>
                    <label><?php print t('Terms of service'); ?></label>
                    <div><?php print fs_apim_markdown_field(fs_apim_get_translated_string($product, ['info'], 'termsOfService')); ?></div>
                  </div>
                <?php endif; ?>
              </div>
            </div>
    ...
    <div class="bottomBorder plansSection navigate-plans">
      ...
      <div class="apiList">
        ...
        <?php foreach ($apis as $api) : ?>
          <div
            class="planapiwrapper planapiwrapper-<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>">
            <a
              onclick='product.toggleplanapi("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>")'
              href='javascript:;'>
              <div class="api highlightText">
                <span
                  class="apiname"><?php print check_plain(fs_apim_get_translated_string($api, ['info'], 'title')); ?><?php if ($showversions == 1) {
            ...
            <div
              class="apicontents apicontents-<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>">
        ...
        $billing = false;
        $fs_apim_billing_enabled = variable_get('fs_apim_billing_enabled', 0);
        if ($fs_apim_billing_enabled == 1) {
      ...
      <div class="plans">
        <div class="plansinner">
          <?php foreach ($product['plans'] as $planName => $plan) : ?>
            ...
            <div class="plan">
              <div
                class="title <?php print $approvalclass; ?>"><?php print check_plain(fs_apim_get_translated_string($plan, array(), 'title')); ?><?php print $approvalimage; ?><?php print $deprecatedstr; ?>
              </div>
              ...
              <div style="padding: 10px;">
                ...
                <?php elseif (!fs_apim_check_is_developer()) : ?>
                ...
                <?php else: ?>
                  <?php if (product_check_product_subscribe($node)): ?>
                    <button type="button" id="planSignupButton"
                            data-href="<?php print url("application/subscribe/" . fs_apim_base64_url_encode($planid) . '/' . $approvestr); ?>"
