...
drupal_add_css(drupal_get_path('module', 'fs_apim') . '/css/mesh.css');
drupal_add_css(drupal_get_path('module', 'fs_apim') . '/css/product.css');
drupal_add_css(drupal_get_path('module', 'fs_apim') . '/css/sunburst.min.css');
drupal_add_css(drupal_get_path('module', 'fs_apim') . '/css/googlecode.min.css');
...
$showversions = variable_get('fs_apim_show_versions', 1);
$fs_apim_enable_api_test = variable_get('fs_apim_enable_api_test', 1);
$fs_apim_soap_codesnippets = variable_get('fs_apim_soap_codesnippets', FALSE);
$fs_apim_soap_swagger_download = variable_get('fs_apim_soap_swagger_download', FALSE);
$codesnippets = variable_get('fs_apim_codesnippets', array(
  'curl' => 1,
  'ruby' => 1,
  'python' => 1,
  'php' => 1,
...
$index = 0;
$apim_session = &_fs_apim_get_apim_session();
...
if (isset($api['x-fs-endpoints'][0]['endpointUrl'])) {
  $default_endpoint = $api['x-fs-endpoints'][0]['endpointUrl'];
}
else {
...
<article id="node-inner-<?php print $node->nid; ?>"
         class="mesh-portal-api <?php print $classes . ' ' . $content['api_apiid'][0]['#markup'] . ' ' . $protocol; ?> apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?> inner clearfix" <?php print $attributes; ?>>
  <div class="navigate-apis">
    <div class="readAndInteract">

      <section
        class="documentation navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>">
        <div class="documentationContent fsSelfClear">
          <div class="titleHeader">
                    ...
                    <div class="field-items">
                      <figure class="clearfix field-item even">
                        <img typeof="foaf:Image" class="image-style-none"
                             src="<?php print file_create_url(drupal_get_path('module', 'fs_apim') . '/images/icons/api/' . api_random_image($node->title)); ?>"
            ...
            <h1 class="name">
              <span><?php print check_plain(fs_apim_get_translated_string($api, ['info'], 'title')); ?> <?php if ($showversions == 1): ?>
                  <span
                    class="version"><?php print check_plain($api['info']['version']); ?></span><?php endif; ?></span>
            </h1>
            <div class="downloadButton">
              <?php if (strtolower($protocol) != 'wsdl' || $fs_apim_soap_swagger_download == TRUE) : ?>
                <a
                  href="<?php print url('fs_apim/swaggerjson/' . fs_apim_base64_url_encode($api['info']['x-fs-name'] . ':' . $api['info']['version'])) ?>"
                  target='_blank'>
          ...
          <div class="info">
              ...
              <?php if (isset($api['tags']) && is_array($api['tags'])) : ?>
                <?php foreach ($api['tags'] as $key=>$tag): ?>
                  <?php if (isset($tag['description'])) {
                    $tagdescr = 'title="' . fs_apim_get_translated_string($api, ['tags', $key], 'description') . '"';
                  }
                  else {
                    $tagdescr = '';
                  } ?>
                  ...
                  <span
                    ...
                    <a
                      onclick='API.selecttag("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>", "<?php print drupal_html_class($tag['name']); ?>")'
                      title="<?php print t('Filter operations by @tag', array('@tag' => fs_apim_get_translated_string($tag, array(), 'name'))); ?>"
                      href='javascript:;'><span
                        class="apiName"><?php print fs_apim_get_translated_string($tag, array(), 'name'); ?></span></a> <a
                      onclick='API.unselecttag("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>")'
            ...
            <?php if (isset($api['info']['description']) && !empty($api['info']['description'])) : ?>
              <div>
                ...
                <div
                  class="markdown"><?php print fs_apim_markdown_field(fs_apim_get_translated_string($api, ['info'], 'description')); ?></div>
              </div>
            <?php endif; ?>
            <?php $docs = render($content['api_attachments']); ?>
            <?php if ((isset($docs) && !empty($docs)) || (isset($api['externalDocs']) && !empty($api['externalDocs']) && isset($api['externalDocs']['url'])) || isset($api['x-fs-configuration']['externalDocs'])) : ?>
              <div>
                ...
                <?php if (isset($api['externalDocs'])) : ?>
                  <div><a href="<?php print $api['externalDocs']['url']; ?>" target='_blank'>
                      <?php if (isset($api['externalDocs']['description'])) {
                        print fs_apim_get_translated_string($api, ['externalDocs'], 'description');
                      }
                      else {
                ...
                <?php endif; ?>
                <?php if (isset($api['x-fs-configuration']['externalDocs'])) : ?>
                  <?php foreach ($api['x-fs-configuration']['externalDocs'] as $key=>$doc) : ?>
                    <div><a href="<?php print $doc['url']; ?>" target='_blank'>
                        <?php if (isset($doc['description'])) {
                          print fs_apim_get_translated_string($api, ['x-fs-configuration', 'externalDocs' ,$key], 'description');
                        }
                        else {
                          print $doc['url'];
                        } ?>
              ...
              <div class="licenseContent">
                ...
                <?php if (isset($api['info']['termsOfService']) && !empty($api['info']['termsOfService'])) : ?>
                  <div>
                    ...
                    <div><?php print fs_apim_get_translated_string($api, ['info'], 'termsOfService'); ?></div>
                  </div>
                <?php endif; ?>
              </div>
            </div>
          ...
          <?php foreach ($api as $childname => $child) : ?>
            <?php if (_fs_apim_startsWith(strtolower($childname), 'x-') && !_fs_apim_startsWith(strtolower($childname), 'x-fs-')) : ?>
              <div class="vendorExtension">
                <label><?php print substr($childname, 2); ?></label>
                <div>
                  <div
                    class="vendorExtension"><?php print fs_apim_render_extension($child); ?></div>
                </div>
              </div>
            <?php endif; ?>
          <?php endforeach; ?>
      ...
      <section class="interact" style="padding: 0;">
        <div
          class="rightHeader stick <?php print $protocol . ' ' . strtolower($fs_apim_soap_codesnippets); ?>">
          <div class="tableWrapper">
            <?php if (($protocol != 'wsdl' || $fs_apim_soap_codesnippets == TRUE) && isset($codesnippets)) : ?>
              <ul class="langs-menu">
                <?php $currentClass = ''; ?>
        ...
        if (module_exists('forum')) {
          $autocreate_forum = variable_get('fs_apim_autocreate_apiforum', 1);
    ...
    <?php if (isset($api['x-fs-endpoints'])) : ?>
        ...
        <section class="documentation">
          <div class="apiEndpointContainer">
            ...
            <div class='apiEndpoints'>
              <?php foreach ($api['x-fs-endpoints'] as $key => $endpoint) : ?>
                <div class='apiEndpoint'>
                  ...
                  <?php if (isset($endpoint['description'])) : ?>
                    <div
                      class='apiEndpointDescr'><?php print fs_apim_markdown_field(fs_apim_get_translated_string($endpoint, array(), 'description')); ?></div>
                  <?php endif; ?>
                </div>
              <?php endforeach; ?>
            </div>
        ...
        <section class="interact">
          <div class="contrast endpointSelect">
            <select name="endpoints"
                    data-api="<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>"
                    class="endpointSelect navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_endpoints">
              <?php foreach ($api['x-fs-endpoints'] as $endpoint)  : ?>
                <option
    ...
    <?php foreach ($api['paths'] as $pathSegment => $path) : ?>
      <div class='pathWrapper'>
        ...
        <?php endif; ?>
        <div
          class='pathDetails navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths'>
          <?php foreach ($path as $verb => $operation) : ?>
            <?php if (in_array(strtoupper($verb), array(
              ...
              <?php endif; ?>
              <div
                class='readAndInteract operation <?php print $optags; ?> opwrapper-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment); ?>_<?php print $verb; ?>'>
                <section
                  class='documentation navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment); ?>'>
                  <div
                    class='operationDetails <?php print $verb; ?> navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment); ?>_<?php print $verb; ?>'>
                    ...
                    <div class='operationContent'>
                            ...
                            <?php if (isset($api['tags']) && is_array($api['tags'])) {
                              foreach ($api['tags'] as $tagkey => $tag) {
                                if (isset($tag['name']) && $tag['name'] == $tagname) {
                                  $tagfound = TRUE;
                                  $tag_title = fs_apim_get_translated_string($tag, array(), 'name');
                                  if (isset($tag['description'])) {
                                    $description = fs_apim_get_translated_string($tag, array(), 'description');
                                  }
                                }
                              }
                              if ($tagfound != TRUE) {
                      ...
                      <?php if (isset($operation['summary']) && !empty($operation['summary'])) : ?>
                        <div>
                          ...
                          <div
                            class="markdown"><?php print fs_apim_markdown_field(fs_apim_get_translated_string($operation, array(), 'summary')); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (isset($operation['description']) && !empty($operation['description'])) : ?>
                        <div>
                          <label><?php print t('Description'); ?></label>
                          <div
                            class="markdown"><?php print fs_apim_markdown_field(fs_apim_get_translated_string($operation, array(), 'description')); ?></div>
                        </div>
                      <?php endif; ?>
                        ...
                        <div>
                          ...
                          <div><a
                              ...
                              <?php if (isset($operation['externalDocs']['description'])) {
                                print fs_apim_get_translated_string($api, ['paths', $pathSegment, $verb, 'externalDocs'], 'description');
                              }
                              else {
                                  ...
                                  <div
                                    ...
                                    <?php
                                      if(isset($api['securityDefinitions'][$securityDefName]['description'])) {
                                        print fs_apim_markdown_field($api['securityDefinitions'][$securityDefName]['description']);
                                      }
                                    ?>
                                    <?php if (isset($operation['security'])) {
                                ...
                                <div class='parametersSection listContent'>
                                  <div class='name'><div
                                    ...
                                    <?php if (isset($parameter['description'])): ?>
                                      <div
                                        class='description markdown'><?php print fs_apim_markdown_field(fs_apim_get_translated_string($operation_parameters, [$key], 'description')); ?></div>
                                    <?php endif; ?><div
                                      class='schema'>
                                      <?php if (isset($parameter) && isset($parameter['schema']) && isset($parameter['schema']['$ref'])) : ?>
                                        <a
                                          onclick='API.navigatedefs("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>", "<?php print drupal_html_class(preg_replace("/\W/", "", _fs_apim_ref_to_objectname($parameter['schema']['$ref']))); ?>")'
                                          href='javascript:;'><?php print fs_apim_return_schema($parameter); ?></a>
                                      <?php elseif (isset($parameter) && isset($parameter['schema']['type']) && $parameter['schema']['type'] == "array" && isset($parameter['schema']['items']) && isset($parameter['schema']['items']['$ref'])) : ?>
                                        <a
                                          onclick='API.navigatedefs("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>", "<?php print drupal_html_class(preg_replace("/\W/", "", _fs_apim_ref_to_objectname($parameter['schema']['items']['$ref']))); ?>")'
                                          href='javascript:;'><?php print fs_apim_return_schema($parameter); ?></a>
                                      <?php elseif (isset($parameter) && (isset($parameter['schema']['type']) && !empty($parameter['schema']['type'])) || (isset($parameter['type']) && !empty($parameter['type']))) : ?>
                                        <?php
                                        $inline_schema = fs_apim_return_inline_schema($parameter);
                                        ...
                                        ?>
                                      <?php elseif (isset($parameter) && (isset($parameter['schema']['type']) && !empty($parameter['schema']['type'])) || (isset($parameter['type']) && !empty($parameter['type']))) : ?>
                                        <?php $inline_schema = fs_apim_return_inline_schema($parameter);
                                        if (isset($inline_schema) && !empty($inline_schema)) {
                                          print "<pre
                                          class='inlineSchema'><code>" . json_encode($inline_schema, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . "</code></pre>";
                                        }
                                        ?>
                                      <?php else: ?>
                                        <?php print fs_apim_return_schema($parameter); ?>
                                      <?php endif; ?>
                                    </div>
                                  </div>
                                </div>
                          ...
                          <?php foreach ($operation['responses'] as $code => $response) : ?>
                            ...
                            <div class='responsesSection listContent'>
                              <div class='code'><?php print $code; ?>
                                  ...
                                  class='description markdown'><?php print fs_apim_markdown_field(fs_apim_get_translated_string($response, array(), 'description')); ?></div><div
                                  class='schema'>
                                  <?php if (isset($response) && isset($response['$ref'])) {
                                    $ref = _fs_apim_ref_to_objectname($response['$ref']);
                                    $response = $api['responses'][$ref];
                                  }
                                  ?>
                                  <?php if (isset($response) && isset($response['schema']) && isset($response['schema']['$ref'])) : ?>
                                    <a
                                      onclick='API.navigatedefs("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>", "<?php print drupal_html_class(preg_replace("/\W/", "", _fs_apim_ref_to_objectname($response['schema']['$ref']))); ?>")'
                                      href='javascript:;'><?php print fs_apim_return_schema($response); ?></a>
                                  <?php elseif (isset($response) && isset($response['schema']['type']) && $response['schema']['type'] == "array" && isset($response['schema']['items']) && isset($response['schema']['items']['$ref'])) : ?>
                                    <a
                                      onclick='API.navigatedefs("<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>", "<?php print drupal_html_class(preg_replace("/\W/", "", _fs_apim_ref_to_objectname($response['schema']['items']['$ref']))); ?>")'
                                      href='javascript:;'><?php print fs_apim_return_schema($response); ?></a>
                                  <?php elseif (isset($expanded_response) && isset($expanded_response['schema']['type']) && !empty($expanded_response['schema']['type'])) : ?>
                                    <?php $response_json = json_encode(fs_apim_return_inline_schema($expanded_response), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE); ?>
                ...
                <section class='interact'>
                  <div
                    class='tabs-content content-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment); ?>_<?php print $verb; ?>'>
                    <?php if (($protocol != 'wsdl' || $fs_apim_soap_codesnippets == TRUE) && isset($codesnippets)) : ?>
                      <div
                        id='tab-content_code_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']) . '_' . preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>'
                        ...
                        <div class="langtab">
                          <?php if (isset($codesnippets['curl']) && $codesnippets['curl'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-curl"
                          ...
                          <?php if (isset($codesnippets['ruby']) && $codesnippets['ruby'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-ruby"
                          ...
                          <?php if (isset($codesnippets['python']) && $codesnippets['python'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-python"
                          ...
                          <?php if (isset($codesnippets['php']) && $codesnippets['php'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-php"
                          ...
                          <?php if (isset($codesnippets['java']) && $codesnippets['java'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-java"
                          ...
                          <?php if (isset($codesnippets['node']) && $codesnippets['node'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-node"
                          ...
                          <?php if (isset($codesnippets['go']) && $codesnippets['go'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-go"
                          ...
                          <?php if (isset($codesnippets['swift']) && $codesnippets['swift'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-swift"
                          ...
                          <?php if (isset($codesnippets['c']) && $codesnippets['c'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-c"
                          ...
                          <?php if (isset($codesnippets['csharp']) && $codesnippets['csharp'] == 1): ?>
                            ...
                            <div
                              id="langtab-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>-csharp"
                    ...
                    <?php endif; ?>
                    <div
                      id='tab-content_example_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']) . '_paths_' . preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>'
                      ...
                      <div class="exampleResponse">
                        <div class="exampleRespData"
                             id="exampleresponse_apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>">
                    ...
                    <?php if ($fs_apim_enable_api_test == 1 && (!isset($api['x-fs-configuration']['testable']) || $api['x-fs-configuration']['testable'] == TRUE)) : ?>
                    <div
                      id='tab-content_try_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']) . '_' . preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>'
                      class='tab-content'>
                      <div
                        class="rightSectionHeading"><?php print t('Try this operation'); ?></div>
                      <form class="testForm"
                            name="request_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']) . "_" . preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>"
                            onsubmit="API.test('<?php print $api['info']['x-fs-name'] . $api['info']['version']; ?>', '<?php print $verb ?>', '<?php print str_replace("'","%27",$pathSegment); ?>');">
                        <input type="password" style="display:none;"/>
                        <input type="submit" style="display: none;"
                               tabindex="-1"/>
                        <div class='contrast'>
                          <?php if (count($api['schemes']) > 1 && !isset($api['x-fs-endpoints'])) : ?>
                            <select name="scheme">
                            ...
                            <div class="contrast apimAuthUrl hidden">
                              ...
                              <div class="parameter buttons">
                                <div
                                  ...
                                  <button type="button"
                                          onclick="API.authorize('<?php print $api['info']['x-fs-name'] . $api['info']['version']; ?>', '<?php print $verb ?>', '<?php print str_replace("'","%27",$pathSegment); ?>')"
                                ...
                                <div
                                  ...
                                  <button type="button"
                                          ...
                                          onclick="API.getToken('<?php print $api['info']['x-fs-name'] . $api['info']['version']; ?>', '<?php print $verb ?>', '<?php print str_replace("'","%27",$pathSegment); ?>')"
                                ...
                                <div
                                  ...
                                  <button type="button"
                                          onclick="API.refreshToken('<?php print $api['info']['x-fs-name'] . $api['info']['version']; ?>', '<?php print $verb ?>', '<?php print str_replace("'","%27",$pathSegment); ?>')"
                    ...
                    <?php if ($bodypresent == TRUE)  : ?>
                      ...
                      <div class="contrast">
                        <div class='parameter'
                             id='body_apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_paths_<?php print preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>'>
                    ...
                    <button style="margin: 0px 15px 15px 0px;"
                            ...
                            onclick="API.test('<?php print $api['info']['x-fs-name'] . $api['info']['version']; ?>', '<?php print $verb ?>', '<?php print str_replace("'","%27",$pathSegment); ?>');"><?php print t('Call operation'); ?></button>
                    <div class='clearBoth'></div>
                    <div class="responseSection hidden"
                         id="response_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']) . '_' . preg_replace("/\W/", "", $pathSegment) . '_' . $verb; ?>">
    ...
    <?php if ((isset($api['definitions'])) && !empty($api['definitions'])) : ?>
      <div class="definitionsSection bottomBorder">
        <div class="readAndInteract">
          <section
            class="documentation definitionsContent navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?> definitions_toggle_apis-<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>">
            <h2
              class="navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_definitions"><?php print t('Definitions'); ?>
            </h2>
            <div class="definitionsWrapper">
              <?php foreach ($api['definitions'] as $definitionName => $definition) : ?>
                <label
                  class="navigate-apis_<?php print drupal_html_class($api['info']['x-fs-name'] . $api['info']['version']); ?>_definitions_<?php print drupal_html_class(preg_replace("/\W/", "", $definitionName)); ?>"><?php print $definitionName; ?></label>
                <?php if (isset($definition['description'])) : ?>
                  <div
                    class="markdown"><?php print fs_apim_markdown_field(fs_apim_get_translated_string($definition, array(), 'description')); ?></div>
                  ...
                  <div>
                    ...
                    <div><a
                        ...
                        <?php if (isset($definition['externalDocs']['description'])) {
                          print fs_apim_get_translated_string($definition, ['externalDocs'], 'description');
